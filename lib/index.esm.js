...
class NumeratorFlagsManager {
    constructor(configClient, defaultContext, loadPolling = true) {
        this.cacheFlags = {};
        this.flags = {};
    ...
    fetchPollingFeatureFlag() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const result = yield this.numeratorClient.fetchPollingFlag(this.defaultContext, this.currentEtag);
                if (result.flags) {
...
const NumeratorProvider = ({ children, configClient, defaultContext, loadPolling = true, }) => {
    ...
    const [errorListeners, setErrorListeners] = useState([]);
    const fetchPollingFeatureFlag = useCallback(() => __awaiter(void 0, void 0, void 0, function* () {
        try {
    ...
    // Register and unregister update listeners
    const handleFlagUpdated = useCallback((callback) => {
        setUpdateListeners((prev) => [...prev, callback]);
        return () => setUpdateListeners((prev) => prev.filter((c) => c !== callback));
    }, []);
    // Register and unregister error listeners
    const handleFlagUpdatedError = useCallback((callback) => {
...
export { FlagStatusEnum, FlagValueTypeEnum, NumeratorClient, NumeratorFlagsManager, NumeratorProvider, addMockedFlag, mockFlags, removeMockedFlag, resetNumeratorMocks, useMockNumeratorProvider, useNumeratorContext };
